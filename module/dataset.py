...
class EEGPreImageDataset(Dataset):
    def __init__(
        ...
        # Metadata (channels/time). Prefer this repo's info.json if present;
        # otherwise fall back to per-subject files (e.g., NICE-EEG dict).
        info_path = os.path.join(eeg_data_dir, "info.json")
        self.info = None
        if os.path.isfile(info_path):
            self.info = json.load(open(info_path, "r"))
        ...
        # Will be set once we load the first subject if info.json is absent.
        self.all_channels = self.info["ch_names"] if self.info is not None else None
        
        self.eeg_data_list = []
        for subject_id in tqdm(subject_ids, desc="Subjects", position=0):
            cache_key = _eeg_cache_key(
            ...
            cache_path = _eeg_cache_path(eeg_data_dir, cache_key)

            eeg_data = None
            if os.path.isfile(cache_path) and not frozen_eeg_prior:
                # Fast path: reuse already-processed EEG (works across separate runs/processes).
                # Note: we disable this when frozen_eeg_prior=True because that mode may apply
                # transforms in __init__, which would otherwise mutate cached arrays.
                eeg_data = np.load(cache_path, mmap_mode="r")
            else:
                subject_dir = os.path.join(eeg_data_dir, f"sub-{subject_id:02}")
                eeg_data_path = _resolve_eeg_file(subject_dir, train=train)
                eeg_container = _load_eeg_container(eeg_data_path)

                # If info.json is missing, try to take ch_names from the file.
                if self.all_channels is None and isinstance(eeg_container, dict) and "ch_names" in eeg_container:
                    self.all_channels = eeg_container["ch_names"]

                eeg_data = _standardize_eeg_array(
                    eeg_container,
                    train=train,
                    num_images_per_object=10,  # Things-EEG convention
                )
                # shape: (1654, 10, 4, 63, 250) after standardization for Things-EEG

                # Resolve channel indices once channel names are known.
                if len(selected_channels) > 0:
                    if self.all_channels is None:
                        raise RuntimeError(
                            "selected_channels was provided, but channel names are unavailable. "
                            "Provide info.json in eeg_data_dir or use NICE-EEG dict files with 'ch_names'."
                        )
                ...
                # Persist to cache for future runs (only for the base, untransformed data).
                if not frozen_eeg_prior:
                    tmp_fd, tmp_path = tempfile.mkstemp(prefix="tmp_", suffix=".npy", dir=os.path.dirname(cache_path))
                    os.close(tmp_fd)
                    try:
                        np.save(tmp_path, eeg_data)
            ...
            # Channels axis depends on average mode:
            # - average: (obj, img, ch, t)
            # - not average: (obj, img, rep, ch, t)
            ...
            # Drop per-subject temporaries aggressively; these NICE-EEG pickled npy files can be multi-GB.
            if "eeg_container" in locals():
                del eeg_container
            gc.collect()
